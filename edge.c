...
/** Positions in the transop array where various transforms are stored.存储各种变换的跨接阵列中的位置。
 *
...
#endif
#define N2N_EDGE_SUP_ATTEMPTS   3       /* Number of failed attmpts before moving on to next supernode.继续下一个超级节点之前失败的攻击次数 */
...
struct n2n_edge
{
    ...
    size_t              sn_idx;                 /**< Currently active supernode. 目前活跃的超级节点。*/
    size_t              sn_num;                 /**< Number of supernode addresses defined. 定义的超级节点地址数。*/
    n2n_sn_name_t       sn_ip_array[N2N_EDGE_NUM_SUPERNODES];
    int                 sn_wait;                /**< Whether we are waiting for a supernode response.我们是否在等待超级节点的响应。 */

    n2n_community_t     community_name;         /**< The community. 16 full octets. 社区名。 16个字节。*/
    char                keyschedule[N2N_PATHNAME_MAXLEN];
    int                 null_transop;           /**< Only allowed if no key sources defined.只有在没有定义关键源的情况下才允许。 */

    int                 udp_sock;
    int                 udp_mgmt_sock;          /**< socket for status info. 套接字状态信息。*/

    tuntap_dev          device;                 /**< All about the TUNTAP device 关于TUNTAP设备*/
    int                 dyn_ip_mode;            /**< Interface IP address is dynamically allocated, eg. DHCP. 接口IP地址是动态分配的，例如。DHCP。*/
    int                 allow_routing;          /**< Accept packet no to interface address. 接收数据包no到接口地址。*/
    int                 drop_multicast;         /**< Multicast ethernet addresses.组播以太网地址。 */

    n2n_trans_op_t      transop[N2N_MAX_TRANSFORMS]; /* one for each transform at fixed positions 一个用于在固定位置的每个变换*/
    size_t              tx_transop_idx;         /**< The transop to use when encoding. 编码时使用的跨接站。*/

    struct peer_info *  known_peers;            /**< Edges we are connected to. 我们连接的边。*/
    struct peer_info *  pending_peers;          /**< Edges we have tried to register with. 我们试图注册的边。*/
    time_t              last_register_req;      /**< Check if time to re-register with super 检查是否有时间重新注册超级*/
    size_t              register_lifetime;      /**< Time distance after last_register_req at which to re-register. 在last_register_req后重新注册的时间距离。*/
    time_t              last_p2p;               /**< Last time p2p traffic was received. 上次收到p2p流量。*/
    time_t              last_sup;               /**< Last time a packet arrived from supernode. 从超级节点到达的最后一个数据包*/
    size_t              sup_attempts;           /**< Number of remaining attempts to this supernode. 对这个超级节点的剩余尝试次数。*/
    n2n_cookie_t        last_cookie;            /**< Cookie sent in last REGISTER_SUPER. */

    time_t              start_time;             /**< For calculating uptime 计算正常运行时间*/
...
/** Return the IP address of the current supernode in the ring.返回当前超级节点的IP地址。 */
...
/* parse the configuration file 解析配置文件 */
...
/* Create the argv vector 创建argv向量*/
...
/** Initialise an edge to defaults.将边缘初始化为默认值。
 *
...
static int edge_init(n2n_edge_t * eee)
{
    ...
    eee->tx_transop_idx = N2N_TRANSOP_NULL_IDX; /* No guarantee the others have been setup不保证其他人已经设置 */

    eee->daemon = 1;    /* By default run in daemon mode. 默认情况下以守护进程运行。*/
    eee->re_resolve_supernode_ip = 0;
    /* keyschedule set to NULLs by memset  keyschedule通过memset设置为NULL*/
    /* community_name set to NULLs by memset  community_name通过memset设置为NULL*/
    eee->null_transop   = 0;
    eee->udp_sock       = -1;
    eee->udp_mgmt_sock  = -1;
    eee->dyn_ip_mode    = 0;
...
/* Called in main() after options are parsed. 在选项被解析后，在main（）中调用。*/
...
/** Send a datagram to a socket defined by a n2n_sock_t 将数据报发送到由n2n_sock_t定义的套接字*/
...
/** Send a REGISTER packet to another edge.发送一个REGISTER数据包到另一个边缘。 */
...
/** Send a REGISTER_SUPER packet to the current supernode.发送REGISTER_SUPER数据包到当前的超级节点。 */
...
/** Send a REGISTER_ACK packet to a peer edge. 将REGISTER_ACK数据包发送到对等端。*/
...
/** @brief Check to see if we should re-register with the supernode.检查我们是否应该重新注册超级节点。
 *
 *  This is frequently called by the main loop.这通常被主循环调用。
 */
 ...
 *  corresponding packet to the cooked socket.从TAP接口读取单个数据包，处理并将相应的数据包写入已熟的套接字。
 */
...
/** Read a datagram from the main UDP socket to the internet. 从主UDP套接字读取数据报到互联网。*/
static void readFromIPSocket( n2n_edge_t * eee )
{
 ...
 *  file descriptors. 在Windows中启动第二个线程，因为TUNTAP接口不会公开文件描述符。*/
...
/** Resolve the supernode IP address.解析超级节点IP地址。
 *
...
int main(int argc, char* argv[])
{
    ...
    n2n_edge_t eee; /* single instance for this program  此程序的单一实例*/
    ...
    /*  strip trailing spaces 带拖尾空格*/
    while(strlen(linebuffer) && linebuffer[strlen(linebuffer)-1]==' ')
        linebuffer[strlen(linebuffer)-1]= '\0';

    /* build the new argv from the linebuffer 从线缓冲区构建新的argv*/
...
static int run_loop(n2n_edge_t * eee )
{
    ...
    while(keep_running)
    {
        ...
        /* Make sure ciphers are updated before the packet is treated. 确保密码在数据包被处理之前被更新*/
        ...
        if(rc > 0)
        {
            /* Any or all of the FDs could have input; check them all. 任何或所有FD可以输入; 检查所有。*/

            if(FD_ISSET(eee->udp_sock, &socket_mask))
            {
                /* Read a cooked socket from the internet socket. Writes on the TAP
                 * socket. */
                readFromIPSocket(eee);//从主UDP套接字读取数据报到互联网。
            }
            ...
            if(FD_ISSET(eee->udp_mgmt_sock, &socket_mask))
            {
                /* Read a cooked socket from the internet socket. Writes on the TAP
                 * socket. */
                readFromMgmtSocket(eee, &keep_running);//从管理UDP套接字读取数据报并采取适当的措施。
            }
            ...
            if(FD_ISSET(eee->device.fd, &socket_mask))
            {
                 ...
                 * socket. */
                readFromTAPSocket(eee);//从TAP接口读取单个数据包，处理并将相应的数据包写入已熟的套接字。
            }
#endif
#ifdef N2N_MULTIPLE_SUPERNODES
        ...
        /* Finished processing select data. 完成处理选择数据。*/
        ...
        update_supernode_reg(eee, nowTime);//检查我们是否应该重新注册超级节点。

        numPurged =  purge_expired_registrations( &(eee->known_peers) );//清除过期注册
